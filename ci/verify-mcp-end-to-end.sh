...
#
# Optional env:
#   PORT           — loopback port. Defaults to 0, i.e. an ephemeral port the
#                    OS picks, so concurrent builds on one agent cannot collide.
#   WORK_DIR       — scratch dir (default: mktemp -d).
#   MVN            — maven binary (default: mvn).
#   VENV           — python venv for the MCP (default: the module's .venv).
#
...
MVN="${MVN:-mvn}"
# 0 = an ephemeral port, chosen by the OS and read back from the server.
# A fixed port would collide with a concurrent build on a shared agent,
# and the failure would look like the service refusing to start.
PORT="${PORT:-0}"
...
public class Launcher {
    public static void main(String[] a) throws Exception {
        ...
        s.start();
        System.out.println("READY " + s.getPort());
...
grep -q '^READY ' "${WORK_DIR}/server.log" \
  || { cat "${WORK_DIR}/server.log"; echo "REST service did not start"; exit 1; }
BOUND_PORT="$(awk '/^READY /{print $2; exit}' "${WORK_DIR}/server.log")"
BASE_URL="http://127.0.0.1:${BOUND_PORT}"
say "REST service is on ${BOUND_PORT}"
...
BASE="${BASE_URL}" "${VENV}/bin/python" "${REPO_DIR}/ci/mcp-e2e/drive_tools.py"
...
APP="$(BASE="${BASE_URL}" RUMI_VERSION="${RUMI_VERSION}" \
