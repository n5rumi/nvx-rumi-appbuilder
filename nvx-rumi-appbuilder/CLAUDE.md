...
This file provides guidance to Claude Code when working in this appbuilder
subtree.

## Project Overview

The **Rumi App Builder** is the library + service + MCP surface for
scaffolding and incrementally building Rumi applications. Three sibling
modules under this directory, each a thin layer on the one beneath it:
...
├── CLAUDE.md / PROJECT.md          # This subtree's docs
...
The `pom.xml` at this level is both **aggregator** (its `<modules>` list
is `nvx-rumi-appbuilder-sdk` and, once it lands, `nvx-rumi-appbuilder-rest`)
...
of the appbuilder parent POM is `com.neeve:nvx-os-parent:1.1.5`.
